...
import de.pueski.jrhythm.util.StencilBufferUtils;
...
public class GLShadowOnPlane {
	...
	public GLShadowOnPlane(float[] lightPosition, float[] plane, float[] shadowcolor, Object parent, Method objectDrawFunction) {
        ...
        fShadowMatrix = StencilBufferUtils.allocFloats(fShadowMatrixArray);
	}
	...
	public void drawObject() {
		invoke(parentApp, drawObjectMethod);
	}
    ...
    public void drawShadow() {
        ...
        GL11.glPushAttrib(GL11.GL_CURRENT_BIT | GL11.GL_COLOR_BUFFER_BIT | GL11.GL_TEXTURE_BIT | GL11.GL_LIGHTING_BIT | GL11.GL_DEPTH_BUFFER_BIT);
        {
        	...
        	GL11.glPushMatrix();
        	{
        		...
        		StencilBufferUtils.clearMask();      // clear the stencil buffer
        		StencilBufferUtils.beginMask(1);     // we'll fill the shadow shape with the number 1
        		drawObject();           // draw the object into stencil buffer 
        		StencilBufferUtils.endMask();        // done with mask
        		StencilBufferUtils.activateMask(1);  // now OpenGL will draw only where stencil buffer == 1
