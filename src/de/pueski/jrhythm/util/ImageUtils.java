...
public class ImageUtils {
	...
	public static BufferedImage getImage(String imageLocation) {
		try {
			return ImageIO.read(Thread.currentThread().getContextClassLoader().getResourceAsStream(imageLocation));
		}
		catch (IOException e) {
			log.info("Could not load "+imageLocation);
			return null;
		}
	}
	
	public static BufferedImage crop(BufferedImage image, int crop) {
		
		BufferedImage cropped = new BufferedImage(image.getWidth()-crop, image.getHeight()-crop, image.getType());
		
		for (int x = crop/2; x < image.getWidth()-crop/2;x++) {
			for (int y = crop/2; y < image.getHeight()-crop/2;y++) {
				cropped.setRGB(x-crop/2, y-crop/2, image.getRGB(x, y));
			}			
		}
